...
namespace XFurStudio {
    ...
    public class XFur_System : MonoBehaviour {
        ...
        public static void DefineXFurLegacy() {
            var defines = PlayerSettings.GetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup);
            if (!defines.Contains("XFURDESKTOP_LEGACY")) {
                PlayerSettings.SetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup, defines + ";XFURDESKTOP_LEGACY");
            }
        }
        ...
        private string internalVersion = "1.9";
...
#if UNITY_EDITOR
        public bool[] folds = new bool[256];
#endif
        ...
        void Start() {
            ...
            for (int i = 0; i < materialProfiles.Length; i++) {
                materialProfiles[i].enableFullShadows = false;
                materialProfiles[i].buffShadowLayers = -32;
            }
        }
        ...
        void LateUpdate() {
            ...
            if (rend)
                rend.lightProbeUsage = Camera.main && Camera.main.renderingPath == RenderingPath.Forward ? UnityEngine.Rendering.LightProbeUsage.BlendProbes : UnityEngine.Rendering.LightProbeUsage.Off;


            for (int i = 0; i < materialProfiles.Length; i++) {

                if (materialProfiles[i].furmatType == 2 && materialProfiles[i].furmatShadowsMode != 0 && materialProfiles[i].buffShadowLayers != materialProfiles[i].fullShadowLayers) {
                    materialProfiles[i].enableFullShadows = false;
                    materialProfiles[i].buffShadowLayers = materialProfiles[i].fullShadowLayers;
                    GenerateSubrenders(materialProfiles[i]);
                    ApplyFurProperties();
                }

                if (materialProfiles[i].furmatType == 2 && materialProfiles[i].furmatShadowsMode != 0 && !materialProfiles[i].enableFullShadows
#if UNITY_EDITOR
                    || !Application.isPlaying
#endif
                    ) {

                    if (materialProfiles[i].furmatShadowsMode != 0 && (materialProfiles[i].subRenders.Length != materialProfiles[i].fullShadowLayers || materialProfiles[i].subRenders[0] == null)) {
                        GenerateSubrenders(materialProfiles[i]);
                        ApplyFurProperties();
                    }
                    ...
                    foreach (Renderer r in materialProfiles[i].subRenders) {
                        if (r != null) {
                            ...
                            r.lightProbeUsage = Camera.main && Camera.main.renderingPath == RenderingPath.Forward ? UnityEngine.Rendering.LightProbeUsage.BlendProbes : UnityEngine.Rendering.LightProbeUsage.Off;
                    ...
                    materialProfiles[i].enableFullShadows = true;
                }
            }
            ...
            if (Application.isPlaying) {
                ...
                if (lodModule.State == XFurModuleState.Disabled || lodModule.XFur_LODLevel > 0 || !physicsModule.affectedByLOD) {
                    physicsModule.Module_Execute();
                }
                if (lodModule.State == XFurModuleState.Disabled || lodModule.XFur_LODLevel > 0 || !fxModule.affectedByLOD) {
...
#else
            if (PrefabUtility.GetPrefabType(this.gameObject) == PrefabType.None) {
#endif 
#endif
                ...
                if ((!Application.isPlaying || GameObject.FindObjectsOfType<XFur_System>().Length == 0) && materialReferences != null && materialReferences.Count > 0) {
                    for (int m = 0; m < materialProfiles.Length; m++) {
                        materialProfiles[m].enableFullShadows = false;
                        if (materialProfiles[m].originalMat && materialReferences.ContainsKey(materialProfiles[m].originalMat)) {
                            for (int i = 0; i < materialReferences[materialProfiles[m].originalMat].Length; i++) {
                                if (materialReferences[materialProfiles[m].originalMat][i])
        ...
        public void CopyFurProperties(XFur_MaterialProperties src, ref XFur_MaterialProperties dest) {
            if (src.furmatType != 2 || dest.furmatType != 2)
                return;
        ...
        //======================================================================
        /// <summary>
        /// WARNING : This function will soon stop being public. Do not use.
        /// </summary>
                ...
                if (!Application.isPlaying || !instancedMode) {
                    if (!materialReferences.ContainsKey(mat.originalMat) || materialReferences[mat.originalMat] == null || materialReferences[mat.originalMat].Length != database.highQualityShaders.Length + database.lowQualityShaders.Length + 2 || materialReferences[mat.originalMat][0] == null) {
                        if (!materialReferences.ContainsKey(mat.originalMat)) {
                            ...
                            mats[database.highQualityShaders.Length + database.lowQualityShaders.Length] = new Material(database.DynamicShellSkin);
                            ...
                            mats[database.highQualityShaders.Length + database.lowQualityShaders.Length + 1] = new Material(database.DynamicShellShader);
        ...
        void OnWillRenderObject() {
            ...
            for (int i = 0; i < materialProfiles.Length; i++) {
                if (materialProfiles[i].furmatType == 0) {
            ...
            foreach (XFur_MaterialProperties mat in materialProfiles) {
                ...
                if (mat.furmatType == 2) {
                    ...
                    if (!mat.originalSynch && !Application.isPlaying) {
                        if (materialReferences.ContainsKey(mat.originalMat)) {
                            foreach (Material samp in materialReferences[mat.originalMat]) {
                                samp.CopyPropertiesFromMaterial(mat.originalMat);
                            }
        ...
        public void XFur_SwitchMaterialSamples(XFur_MaterialProperties mat) {
            ...
            if (UnityEditor.PrefabUtility.GetPrefabAssetType(gameObject) == UnityEditor.PrefabAssetType.Regular) {
                for (int i = 0; i < materialProfiles.Length; i++) {
                    sharedMats[i] = materialProfiles[i].originalMat;
                }
        ...
        void SynchFurProperties(XFur_MaterialProperties mat) {
            ...
            if (!database || mat.furmatType != 2)
                return;

            if (mat.furmatIndex > sharedMats.Length)
                return;
            if (mat != null && sharedMats != null && sharedMats[mat.furmatIndex] != null && !sharedMats[mat.furmatIndex].shader.name.Contains("XFur")) {
                mat.furmatType = 0;
            }
            ...
            if (mat.originalMat && !materialReferences.ContainsKey(mat.originalMat)) {
                UpdateSharedData(mat);
            }
            ...
            if (!Application.isPlaying || !instancedMode) {
#if UNITY_2018_1_OR_NEWER
					rend.GetPropertyBlock( m, mat.furmatIndex );
					var furs = 0;
#else
                ...
                m.SetFloat("_SkinMode", furs > 1 ? mat.originalMat.GetFloat("_SkinMode") : mat.furmatRenderSkin);
                ...
                if (mat.furmatReadBaseSkin > 0 && furs < 2) {
                    ...
                    m.SetTexture("_Normalmap", mat.furmatNormalmap ? mat.furmatNormalmap : bumpEmpty ? bumpEmpty : Texture2D.blackTexture);
                    ...
                    m.SetTexture("_Normalmap", mat.originalMat.GetTexture("_Normalmap") ? mat.originalMat.GetTexture("_Normalmap") : bumpEmpty ? bumpEmpty : Texture2D.blackTexture);
                ...
                if (FurMaterials > 1) {
                    ...
                    for (int mainMat = 0; mainMat < sharedMats.Length; mainMat++) {
                        if (materialProfiles[mainMat].subRenders.Length > 0) {
                            for (int f = 0; f < materialProfiles[mainMat].subRenders.Length; f++) {
                                if (materialProfiles[mainMat].subRenders[f] && materialProfiles[mainMat].subRenders[f].sharedMaterial && materialProfiles[mainMat].subRenders[f].sharedMaterial.HasProperty("_FurStep"))
                                    materialProfiles[mainMat].subRenders[f].sharedMaterial.SetFloat("_FurStep", materialProfiles[mainMat].fullShadowLayers);
                                else if (materialProfiles[mainMat].subRenders[f] && !materialProfiles[mainMat].subRenders[f].sharedMaterial)
                                    materialProfiles[mainMat].subRenders[f].sharedMaterial = materialProfiles[mainMat].originalMat;

                                if (sharedMats[mainMat] && materialProfiles[mainMat].originalMat && sharedMats[mainMat] != materialProfiles[mainMat].originalMat)
                        ...
                        if (sharedMats[mainMat] && sharedMats[mainMat].HasProperty("_FurStep"))
                            sharedMats[mainMat].SetFloat("_FurStep", materialProfiles[mainMat].fullShadowLayers);
                        else if (!sharedMats[mainMat])
                            sharedMats[mainMat] = materialProfiles[mainMat].originalMat;

                        if (sharedMats[mainMat] != materialProfiles[mainMat].originalMat)
                            sharedMats[mainMat].CopyPropertiesFromMaterial(materialProfiles[mainMat].originalMat);
                    }
                }
                ...
                material.SetFloat("_SkinMode", furs > 1 ? mat.originalMat.GetFloat("_SkinMode") : mat.furmatRenderSkin);
                ...
                if (mat.furmatReadBaseSkin > 0 && furs < 2) {
                    ...
                    material.SetTexture("_Normalmap", mat.furmatNormalmap ? mat.furmatNormalmap : bumpEmpty ? bumpEmpty : Texture2D.blackTexture);
                    ...
                    material.SetTexture("_Normalmap", mat.originalMat.GetTexture("_Normalmap") ? mat.originalMat.GetTexture("_Normalmap") : bumpEmpty ? bumpEmpty : Texture2D.blackTexture);
        ...
        public void SetPropertyBlock(MaterialPropertyBlock block, int index) {
            ...
            if (block == null || index < 0 || !rend || sharedMats.Length <= index) {
                ...
                if (materialProfiles[index].furmatShadowsMode == 0) {
                    ...
                    if (materialProfiles[index].furmatType != 2)
                        return;
#if UNITY_2018_1_OR_NEWER
					rend.SetPropertyBlock( block, index );
#else
                    ...
                    for (int i = 0; i < materialProfiles[index].subRenders.Length; i++) {
                        if (materialProfiles[index].subRenders[i] != null) {
                            block.SetFloat("_ShadowLayerIndex", i);
                            block.SetFloat("_FurStep", materialProfiles[index].fullShadowLayers);
        ...
        /// WARNING : This function will soon stop being public. Do not use.
        /// </summary>
        public void GenerateSubrenders(XFur_MaterialProperties mat) {
            ...
            foreach (Component c in baseRenderer.GetComponents(typeof(Component))) {
                if (c.GetType() != typeof(Transform) && c.GetType() != typeof(MeshRenderer) && c.GetType() != typeof(SkinnedMeshRenderer) && c.GetType() != typeof(MeshFilter)) {
                    DestroyImmediate(c);
                }
            }
            ...
            mat.subRenders = new Renderer[(int)mat.fullShadowLayers];
            ...
            for (int i = 0; i < mat.subRenders.Length; i++) {
                ...
                if (mat.subRenders[i].GetType() == typeof(SkinnedMeshRenderer)) {
                    database.XFur_GetShadowMesh(originalMesh, mat.furmatIndex, new SkinnedMeshRenderer[] { (SkinnedMeshRenderer)mat.subRenders[i] });
                }
                else {
                    database.XFur_GetShadowMesh(originalMesh, mat.furmatIndex, new MeshFilter[] { (MeshFilter)mat.subRenders[i].GetComponent<MeshFilter>() });
                }
            }
        ...
        public void XFur_GenerateLayers() {
            ...
            if (Mesh.GetSubmesh(materialProfileIndex).vertexCount < 15000) {
                GenerateShells(materialProfileIndex, 1);
                layersSoFar += 1;
#if UNITY_EDITOR
                UnityEditor.EditorUtility.DisplayProgressBar("XFur Mesh Patcher 1.0", "Generating Fur Data", 1.0f / materialProfiles[materialProfileIndex].fullShadowLayers * layersSoFar);
#endif
            }
            else {
#if UNITY_EDITOR
        ...
        void GenerateShells(int group, int shells) {
            ...
            for (int i = 0; i < shells; i++) {
                ...
                var tempBones = new List<Transform>(physicsModule.bones);

                var realBones = GetComponent<SkinnedMeshRenderer>() ? GetComponent<SkinnedMeshRenderer>().bones : new Transform[0];

                for (int v = 0; v < mesh.vertexCount; v++) {
                    uv1[v] = new Vector2(uv1[v].x, uv1[v].y);
                    ...
                    if (!physicsModule.autoDetectBones) {
                        boneI = 0;
                        foreach (Transform bone in tempBones) {
                            if (realBones[boneI].IsChildOf(bone)) {
                                boneI = tempBones.IndexOf(bone);
                            }
                        }
        ...
        public void XFur_UpdateMeshData(bool forceUpdate = false) {
            ...
            if (GetComponent<MeshFilter>()) {
                ...
                if (database.XFur_ContainsMeshData(originalMesh) == -1 || forceUpdate) {
                    ...
                    var tempBones = new List<Transform>(physicsModule.bones);
                    var realBones = GetComponent<SkinnedMeshRenderer>().bones;
        ...
        public GUIContent[] FurMatGUIS {
            get {
                ...
                for (int i = 0; i < sharedMats.Length; i++) {
                    if (materialProfiles.Length > i && materialProfiles[i].originalMat)
                        names.Add(new GUIContent(i + ". " + materialProfiles[i].originalMat.name.ToUpper()));
                }
                return names.ToArray();
            }
        }
    ...
    public class XFur_MaterialProperties {
        ...
        public float fullShadowLayers = 8;
        public bool enableFullShadows;
        public float buffShadowLayers;
        ...
        public void SynchToOriginalMat() {
            ...
            if (originalMat == buffOriginalMat && furmatType == 2) {
                return;
            }
            else {
